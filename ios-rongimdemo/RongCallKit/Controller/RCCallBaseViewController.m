...
#pragma mark - init FUAPIDemoBar
-(FUAPIDemoBar *)demoBar {
    if (!_demoBar) {
        _demoBar = [[FUAPIDemoBar alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 320, self.view.frame.size.width, 164)];
...
- (void)cameraSwitchButtonClicked {
    ...
    if ([self.callSession switchCameraMode])
    {
        ...
        [FUManager shareManager].flipx = _backCamera;
    }
    //[_cameraSwitchButton setSelected:_backCamera];
}
